...
class DocPageRenderer(mistune.Renderer):
    ...
    def header(self, text, level, raw=None):
        if not HEADER_PAT.match(raw):
            print(
                "invalid header (does not match pattern {}): {}".format(
                    HEADER_PAT_REGEX, raw
                )
            )
        if text != raw:
        ...
        if True:  # render anchors besides headline elements
            ...
            res = """<h{level} id="{anchor}">{text}<a class="headerlink" title="Permalink to this headline" href="#{anchor}">¶</a></h{level}>""".format(
    ...
    def link(self, link, title, content):
        if not (
            link.startswith("http") or link.startswith("/") or link.startswith("#")
        ):
            if self._prefix:
...
class DocPages:
    def __init__(self, docroot, extensions=[".md"], debug=False):
        ...
        print(
            "processed {} files: {} ok, {} error".format(
                total, len(self._pages), errors
            )
        )
