...
from crossbar.router.test.helpers import (
    make_router,
    connect_application_session,
    add_realm_to_router,
)
...
def build_mqtt_server():
    ...
    router = add_realm_to_router(
        router_factory, session_factory, realm_name="mqtt", realm_options={}
    )
    ...
    router.add_role(
        RouterRoleStaticAuth(
            router, "mqttrole", default_permissions=default_permissions
        )
    )
    ...
    class AuthenticatorSession(ApplicationSession):
        ...
        def onJoin(self, details):
            ...
            def tls(realm, authid, details):
                ACCEPTED_CERTS = set(
                    ["95:1C:A9:6B:CD:8D:D2:BD:F4:73:82:01:55:89:41:12:9C:F8:AF:8E"]
                )

                if (
                    "client_cert" not in details["transport"]
                    or not details["transport"]["client_cert"]
                ):
                    raise ApplicationError(
                        "com.example.no_cert", "no client certificate presented"
                    )
                ...
                if sha1 not in ACCEPTED_CERTS:
                    raise ApplicationError(
                        "com.example.invalid_cert",
                        "certificate with SHA1 {} denied".format(sha1),
                    )
                else:
    ...
    options = {
        "options": {
            "realm": "mqtt",
            "role": "mqttrole",
            "payload_mapping": {"": {"type": "native", "serializer": "json"}},
            "auth": {
                "ticket": {
                    "type": "dynamic",
    ...
    def _test_basic_publish(self):
        ...
        client_transport.write(
            Connect(
                ...
                username="test123",
                password="password",
                flags=ConnectFlags(clean_session=False, username=True, password=True),
            ).serialise()
        )
        mqtt_pump.flush()
    ...
    def _test_retained(self):
        ...
        self.assertEqual(
            ...
            Publish(
                ...
                retain=True,
                topic_name="com/test/wamp",
                payload=json.dumps({}, sort_keys=True).encode("utf8"),
            ).serialise(),
        )
    ...
    def _test_lastwill(self):
        ...
        client_transport.write(
            Connect(
                ...
                flags=ConnectFlags(
                    clean_session=False, username=True, password=True, will=True
                ),
            ).serialise()
        )
