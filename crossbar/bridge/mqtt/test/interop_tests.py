...
def test_connect(host, port):
    record = [
        Frame(
            send=True,
            data=Connect(
                client_id="test_cleanconnect", flags=ConnectFlags(clean_session=True)
            ),
        ),
...
def test_quirks_mode_connect(host, port):
    record = [
        Frame(
            send=True, data=b"\x10\x15\x00\x04MQTT\x04\x02\x00x\x00\x07testqrk\x00\x00"
        ),
        Frame(send=False, data=ConnACK(session_present=False, return_code=0)),
        Frame(send=True, data=Disconnect()),
        ConnectionLoss(),
...
def test_reserved_packet_15(host, port):
    record = [
        Frame(
            send=True,
            data=Connect(
                client_id="test_reserved15", flags=ConnectFlags(clean_session=True)
            ),
        ),
        Frame(send=False, data=ConnACK(session_present=False, return_code=0)),
        Frame(
...
def test_reserved_packet_0(host, port):
    record = [
        Frame(
            send=True,
            data=Connect(
                client_id="test_reserved0", flags=ConnectFlags(clean_session=True)
            ),
        ),
        Frame(send=False, data=ConnACK(session_present=False, return_code=0)),
        Frame(
...
def test_uninvited_puback(host, port):
    record = [
        Frame(
            send=True,
            data=Connect(
                client_id="test_puback", flags=ConnectFlags(clean_session=True)
            ),
        ),
        Frame(send=False, data=ConnACK(session_present=False, return_code=0)),
...
def test_uninvited_pubrel(host, port):
    record = [
        Frame(
            send=True,
            data=Connect(
                client_id="test_pubrel", flags=ConnectFlags(clean_session=True)
            ),
        ),
        Frame(send=False, data=ConnACK(session_present=False, return_code=0)),
...
def test_self_subscribe(host, port):
    record = [
        Frame(
            send=True,
            data=Connect(
                client_id="test_selfsub", flags=ConnectFlags(clean_session=True)
            ),
        ),
        Frame(send=False, data=ConnACK(session_present=False, return_code=0)),
        Frame(
...
def test_qos2_send_wrong_confirm(host, port):
    record = [
        Frame(
            send=True,
            data=Connect(
                client_id="test_wrong_confirm_qos2",
...
def test_qos1_send_wrong_confirm(host, port):
    record = [
        Frame(
            send=True,
            data=Connect(
                client_id="test_wrong_confirm_qos1",
                flags=ConnectFlags(clean_session=True),
            ),
        ),
        Frame(send=False, data=ConnACK(session_present=False, return_code=0)),
