...
class Dealer(object):
    ...
    def __init__(self, router, reactor, options=None):
        ...
        self._cancel_timers = txaio.make_batched_timer(
            1
        )  # timeouts have to be integers anyway
        ...
        self._option_uri_strict = (
            self._options.uri_check == RouterOptions.URI_CHECK_STRICT
        )
    ...
    def detach(self, session):
        ...
        if session in self._caller_to_invocations:
            ...
            for invoke in outstanding:  # type: InvocationRequest
                if invoke.canceled:
                    continue
                if (
                    invoke.callee is invoke.caller
                ):  # if the calling itself - no need to notify
                    continue
        ...
        if session in self._session_to_registrations:
            ...
            raise NotAttached(
                "session with ID {} not attached".format(session._session_id)
            )
        ...
        if registration:
            ...
            if session in registration.observers:
                ...
                (
                    was_registered,
                    was_last_callee,
                    has_follow_up_messages,
                ) = self._unregister(registration, session, unregister)
            ...
            if not authorization["allow"]:
                ...
                if registration:
                    ...
                    reply = message.Error(
                        ...
                        [
                            "no callee registered for procedure <{0}>".format(
                                call.procedure
                            )
                        ],
                    )
    ...
    def _call(self, session, call, registration, authorization, is_queued_call=False):
        ...
        if registration.extra.invoke in [
            ...
            if callee_extra:
                if (
                    callee_extra.concurrency
                    and callee_extra.concurrency_current >= callee_extra.concurrency
                ):
                    if is_queued_call or (
                        ...
                        reply = message.Error(
                            ...
                            [
                                "maximum concurrency {} of callee/endpoint reached (on non-shared/single registration)".format(
                                    callee_extra.concurrency
                                )
                            ],
                        )
                        ...
                        if (
                            ...
                            if is_queued_call or (
                                ...
                                reply = message.Error(
                                    ...
                                    "crossbar.error.max_concurrency_reached",
                                    [
                                        "maximum concurrency of all callee/endpoints reached (on round-robin registration)".format(
                                            callee_extra.concurrency
                                        )
                                    ],
                                )
        ...
        if authorization.get("disclose", False):
            disclose = True
        elif call.procedure.startswith("wamp.") or call.procedure.startswith(
            "crossbar."
        ):
            disclose = True
        else:
    ...
    def _add_invoke_request(
        ...
        invoke_request = InvocationRequest(
            invocation_request_id, registration, session, call, callee, forward_for
        )
        ...
        if timeout:
            ...
            invoke_request.timeout_call = self._cancel_timers.call_later(
                timeout, _cancel_both_sides
            )
    ...
    def processCancel(self, session, cancel):
        ...
        if (session._session_id, cancel.request) in self._invocations_by_call:
            invocation_request = self._invocations_by_call[
                session._session_id, cancel.request
            ]
            ...
            if cancellation_mode != message.Cancel.SKIP:
                ...
                interrupt = message.Interrupt(
                    invocation_request.id, interrupt_mode, forward_for=forward_for
                )
    ...
    def processYield(self, session, yield_):
        ...
        if yield_.request in self._invocations:
            ...
            if invocation_request.callee is not session:
                raise ProtocolError(
                    "Dealer.onYield(): YIELD received for non-owned request ID {0}".format(
                        yield_.request
                    )
                )
