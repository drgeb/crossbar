        ...
        if self._proto:
            ...
            if data[0:1] == b"\x7F":
                ...
                if not self._factory._rawsocket_factory:
                    self.log.warn(
                        "client wants to talk RawSocket, but we have no factory configured for that"
                    )
                    self.transport.loseConnection()
                else:
                    self.log.debug("switching to RawSocket")
                    self._proto = self._factory._rawsocket_factory.buildProtocol(
                        self._addr
                    )
                ...
                if not self._factory._mqtt_factory:
                    self.log.warn(
                        "client wants to talk MQTT, but we have no factory configured for that"
                    )
                    self.transport.loseConnection()
                else:
                ...
                if len(rl) != 3:
                    self.log.warn(
                        'received invalid HTTP request line for HTTP protocol subswitch: "{request_line}"',
                ...
                request_uri_first_component = [
                    x.strip() for x in request_uri.split("/") if x.strip() != ""
                ]
                ...
                if self._factory._websocket_factory_map:
                    for (
                        uri_component,
                        websocket_factory,
                    ) in self._factory._websocket_factory_map.items():
                ...
                if not self._proto:
                    ...
                    if self._factory._web_factory:
                        ...
                        self.log.debug(
                            "switching to HTTP/Web on Request-URI {request_uri}",
                            request_uri=request_uri,
                        )
                        self._proto = self._factory._web_factory.buildProtocol(
                            self._addr
                        )
                        ...
                        self.log.warn(
                            "client wants to talk HTTP/Web, but we have no factory configured for that"
                        )
                        self.transport.loseConnection()
                        return
                else:
                    ...
                    self.log.debug(
                        "switching to HTTP/WebSocket on Request-URI {request_uri}",
                        request_uri=request_uri,
                    )
