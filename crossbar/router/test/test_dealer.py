...
class TestDealer(unittest.TestCase):
    ...
    def test_call_cancel(self):
        ...
        dealer.processRegister(
            session,
            message.Register(
                1, "com.example.my.proc", "exact", message.Register.INVOKE_SINGLE, 1
            ),
        )
    ...
    def test_call_cancel_two_sessions(self):
        ...
        dealer.processRegister(
            session0,
            message.Register(
                1, "com.example.my.proc", "exact", message.Register.INVOKE_SINGLE, 2
            ),
        )
    ...
    def test_yield_on_unowned_invocation(self):
        ...
        register = message.Register(
            1, "com.example.some.call", "exact", message.Register.INVOKE_SINGLE, 1
        )
    ...
    def test_concurrency_with_error(self):
        ...
        error = message.Error(
            message.Call.MESSAGE_TYPE, invocation_msg.request, "wamp.error.foo"
        )
        ...
        dealer.processCall(
            caller_session, message.Call(3, "com.example.my.proc", ["foo"])
        )
        invocation_msg = callee_messages[-1]
        self.assertIsInstance(invocation_msg, message.Invocation)
