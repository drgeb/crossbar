...
def make_router():
    ...
    websocket_server_factory = WampWebSocketServerFactory(
    ...
    server_factory = UniSocketServerFactory(
        websocket_factory_map={"/": websocket_server_factory},
...
def add_realm_to_router(
    router_factory, session_factory, realm_name="default", realm_options={}
):
    ...
    router.add_role(
        RouterRoleStaticAuth(
            router, "anonymous", default_permissions=default_permissions
        )
    )
...
def connect_application_session(
    server_factory, MyApplicationSession, component_config=None
):
    """
