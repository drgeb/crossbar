...
class PendingAuthScram(PendingAuth):
    ...
    def hello(self, realm, details):
        ...
        def on_authenticate_ok(principal):
            self._salt = binascii.a2b_hex(
                principal["salt"]
            )  # error if no salt per-user
    ...
    def authenticate(self, signed_message):
        ...
        auth_message = (
            "{client_first_bare},{server_first},{client_final_no_proof}".format(
                client_first_bare="n={},r={}".format(
                    saslprep(self._authid), client_nonce
                ),
                server_first="r={},s={},i={}".format(
                    server_nonce, salt, self._iterations
                ),
        ...
        client_signature = hmac.new(
            self._stored_key, auth_message.encode("ascii"), hashlib.sha256
        ).digest()
        ...
        server_signature = hmac.new(
            self._server_key, auth_message.encode("ascii"), hashlib.sha256
        ).digest()
        ...
        self.log.error(
            "SCRAM authentication failed for '{authid}'", authid=self._authid
        )
