...
class PendingAuthCryptosign(PendingAuth):
    ...
    def _compute_challenge(self, channel_binding):
        ...
        extra = {"challenge": binascii.b2a_hex(self._challenge).decode("ascii")}
        return extra

    def hello(self, realm, details):
        # the channel binding requested by the client authenticating
        channel_binding = (
            details.authextra.get("channel_binding", None)
                    ...
                    for _authid, _principal in self._config.get(
                        ...
                        if pubkey in _principal["authorized_keys"]:
                            ...
                            if self._authid is None:
                                ...
                                return types.Deny(
                                    message="cannot infer client identity from pubkey: multiple authids in principal database have this pubkey"
                                )
                    if self._authid is None:
                        return types.Deny(
                            message="cannot identify client: no authid requested and no principal found for provided extra.pubkey"
                        )
                else:
                    return types.Deny(
    ...
    def authenticate(self, signed_message):
        ...
        try:
            if not isinstance(signed_message, str):
                return types.Deny(
                    message="invalid type {} for signed message".format(
                        type(signed_message)
                    )
                )
            ...
            try:
                ...
                return types.Deny(
                    message="signed message is invalid (not a HEX encoded string)"
                )

            if len(signed_message) != 96:
                return types.Deny(
                    message="signed message has invalid length (was {}, but should have been 96)".format(
                        len(signed_message)
                    )
                )
