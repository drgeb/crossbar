...
class RouterApplicationSession(object):
    ...
    def __init__(self, session, router, authid=None, authrole=None, authextra=None):
        ...
        ), "session must be of class ApplicationSession, not {}".format(
            session.__class__.__name__ if session else type(session)
        )
        assert isinstance(
            router, Router
        ), "router must be of class Router, not {}".format(
            ...
            if self._router.is_attached(self._session):
                ...
                def detach(sess):
                    try:
                        ...
                        self.log.warn(
                            'cannot detach session "{}": session not currently attached'.format(
    ...
    def send(self, msg):
        ...
        #
            ...
            #
            raise Exception(
                "RouterApplicationSession.send: unhandled message {0}".format(msg)
            )
...
class RouterSession(BaseSession):
    ...
    def onOpen(self, transport):
        ...
        if channel_id:
            self._transport._transport_info["channel_id"] = binascii.b2a_hex(
                channel_id
            ).decode("ascii")
    ...
    def onMessage(self, msg):
        ...
        if self._session_id is None:
            ...
            if isinstance(msg, message.Hello):
                ...
                def _on_error(err):
                    self.log.warn(
                        "{klass}.onMessage(msg={msg}) -> callback user onHello raised {err}",
                        klass=self.__class__.__name__,
                        msg=msg,
                        err=err,
                    )
    ...
    def onClose(self, wasClean):
        ...
        """
        self.log.debug(
            "{klass}.onClose(was_clean={was_clean})",
            klass=self.__class__.__name__,
            was_clean=wasClean,
        )
        ...
        if self._service_session:
            ...
            self._service_session.publish(
                "wamp.session.on_stats", session_info_short, session_stats
            )
    ...
    def _swallow_error_and_abort(self, fail):
        ...
        reply = message.Abort(
            "wamp.error.authorization_failed", "Internal server error"
        )
        self._transport.send(reply)
    ...
    def onHello(self, realm, details):
        ...
        try:
            ...
            if hasattr(self._transport, "_authrealm") and self._transport._authrealm:
                if "cookie" in authmethods:
                    ...
                    if hasattr(self._transport, "_cbtid"):
                        self._transport.factory._cookiestore.setAuth(
                            self._transport._cbtid, None, None, None, None, None
                        )
                else:
            ...
            if self._transport._authid is not None and (
                self._transport._authmethod == "trusted"
                ...
                if allow:
                    ...
                    return types.Deny(
                        ...
                        message="session was previously authenticated (via transport), but role '{}' no longer exists on realm '{}'".format(
                            self._transport._authrole, realm
                        ),
                    )
                ...
                if not auth_config:
                    ...
                    if "anonymous" not in authmethods:
                        return types.Deny(
                            ...
                            message="cannot authenticate using any of the offered authmethods {}".format(
                                authmethods
                            ),
                        )
                    ...
                    if not realm:
                        return types.Deny(
                            ApplicationError.NO_SUCH_REALM, message="no realm requested"
                        )
                    ...
                    for authmethod in authmethods:
                        ...
                        # invalid authmethod
                        if (
                            authmethod not in AUTHMETHODS
                            and authmethod not in extra_auth_methods
                        ):
                            self.log.debug("Unknown authmethod: {}".format(authmethod))
                            return types.Deny(
                                message='invalid authmethod "{}"'.format(authmethod)
                            )
                        ...
                        if (
                            authmethod not in AUTHMETHOD_MAP
                            and authmethod not in extra_auth_methods
                        ):
                            self.log.debug(
    ...
    def onAuthenticate(self, signature, extra):
        ...
        """
        self.log.debug(
            "onAuthenticate: {signature} {extra}", signature=signature, extra=extra
        )
        ...
        if self._pending_auth:
            ...
            if (
                isinstance(self._pending_auth, PendingAuthTicket)
    ...
    def onJoin(self, details):
        ...
        if self._service_session:
            ...
            if "stats" in realm_config:
                rated_message_size = realm_config["stats"].get(
                    "rated_message_size", 512
                )
                trigger_after_rated_messages = realm_config["stats"].get(
                    "trigger_after_rated_messages", 0
                )
                trigger_after_duration = realm_config["stats"].get(
                ...
                assert (
                    type(rated_message_size) == int
                    and rated_message_size > 0
                    and rated_message_size % 2 == 0
                )
                ...
                def on_stats(stats):
                    ...
                    stats["last"] = False
                    self._service_session.publish(
                        "wamp.session.on_stats", session_info_short, stats
                    )

                self._transport._serializer.RATED_MESSAGE_SIZE = rated_message_size
                self._transport._serializer.set_stats_autoreset(
                    trigger_after_rated_messages, trigger_after_duration, on_stats
                )
...
class RouterSessionFactory(object):
    ...
    def add(self, session, router, authid=None, authrole=None, authextra=None):
        ...
        if session not in self._app_sessions:
            router_session = RouterApplicationSession(
                session, router, authid, authrole, authextra
            )
