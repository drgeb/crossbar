...
class StaticResource(File):
    ...
    def render_GET(self, request):
        if self._cache_timeout is not None:
            request.setHeader(
                b"cache-control",
                "max-age={}, public".format(self._cache_timeout).encode("utf8"),
            )
            request.setHeader(
                b"expires", http.datetimeToString(time.time() + self._cache_timeout)
            )
...
class RouterWebServiceStatic(RouterWebService):
    ...
    def create(transport, path, config):
        ...
        if "directory" in config:
            ...
            if "resource" not in config:
                raise ApplicationError(
                    "crossbar.error.invalid_configuration", "missing resource"
                )
                ...
                try:
                    static_dir = os.path.abspath(
                        pkg_resources.resource_filename(
                            config["package"], config["resource"]
                        )
                    )
            ...
            raise ApplicationError(
                "crossbar.error.invalid_configuration", "missing web spec"
            )
        ...
        resource = static_resource_class(
            static_dir,
        ...
        if fallback:
            resource.childNotFound = File(
                os.path.join(static_dir.decode("ascii"), fallback)
            )
        else:
