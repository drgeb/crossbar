...
class ZipArchiveResource(resource.Resource):
    ...
    def __init__(self, worker, config, path, archive_file):
        ...
        self.log.info(
            "ZipArchiveResource: {zlen} files in ZIP archive", zlen=len(self._zipfiles)
        )
        self.log.debug(
            "ZipArchiveResource files: {filelist}",
            filelist=sorted(self._zipfiles.keys()),
        )
    ...
    def getChild(self, path, request, retry=True):
        ...
        self.log.debug(
            'ZipArchiveResource.getChild - effective search path: "{}"'.format(
                search_path
            )
        )
...
class RouterWebServiceArchive(RouterWebService):
    ...
    def create(transport, path, config):
        ...
        archive_file = os.path.abspath(
            os.path.join(transport.worker.cbdir, config["archive"])
        )
        if os.path.exists(archive_file):
            if os.path.isfile(archive_file):
                ...
                raise Exception(
                    'path "{archive_file}" exists but is not a file'.format(
                        archive_file=archive_file
                    )
                )
        else:
            ...
            if _url.scheme not in ["http", "https"]:
                raise Exception(
                    'invalid scheme "{}" in attribute "archive" for archive file'.format(
                        _url.scheme
                    )
                )
            ...
            if config.get("download", False):
                ...
                log.info(
                    'ZipArchiveResource: downloading from "{source_url}"',
        ...
        if hashes:
            ...
            if h in hashes:
                ...
                raise Exception(
                    'archive "{}" does not match any of configured SHA256 fingerprints {}'.format(
                        archive_file, hashes
                    )
                )
        else:
            log.warn(
                'ZipArchiveResource: archive file "{archive_file}" is unverified',
                archive_file=archive_file,
            )
