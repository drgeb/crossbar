...
class LogTester(ApplicationSession):
    ...
    def onJoin(self, details):
        ...
        config = self.config.extra or {"iterations": 300, "delay": 0.2}

        self.log.info(
            '{prefix} joined realm "{realm}"', prefix=LOG_PREFIX, realm=details.realm
        )
        self.log.info(
            "{prefix} config={config}", prefix=LOG_PREFIX, config=self.config.extra
        )
