...
class BridgeSession(ApplicationSession):
    ...
    def _setup_event_forwarding(self, other):
            ...
            def on_event(*args, **kwargs):
                ...
                try:
                    ...
                    if e.error not in ["wamp.close.normal"]:
                        self.log.warn(
                            "FAILED TO PUBLISH 1: {} {}".format(type(e), str(e))
                        )
                    return
                except Exception as e:
                    if not ERRMSG:
                        self.log.warn(
                            "FAILED TO PUBLISH 2: {} {}".format(type(e), str(e))
                        )
                        ERRMSG = True
                    return
            ...
            while n > 0:
                try:
                    sub = yield other.subscribe(
                        on_event, uri, options=SubscribeOptions(details=True)
                    )
                except:
                    self.log.failure()
        ...
        def on_subscription_delete(session_id, sub_id, details=None):
            ...
            self.log.debug(
                "{other} unsubscribed from {uri}".format(other=other, uri=uri)
            )
...
class RLinkLocalSession(BridgeSession):
    ...
    def onConnect(self):
        ...
        self.join(
            self.config.realm, authid=self.config.extra["rlink"], authextra=authextra
        )
...
class RLinkRemoteSession(BridgeSession):
    ...
    def onChallenge(self, challenge):
        self.log.debug(
            "{klass}.onChallenge(challenge={challenge})",
            klass=self.__class__.__name__,
            challenge=challenge,
        )
        ...
        if challenge.method == "cryptosign":
            ...
            signed_challenge = self._rlink_manager._controller._node_key.sign_challenge(
                self, challenge
            )
...
class RLink(object):
    def __init__(
        self, id, config, started=None, started_by=None, local=None, remote=None
    ):
        assert type(id) == str
...
class RLinkManager(object):
    ...
    def start_link(self, link_id, link_config, caller):
        ...
        connecting_endpoint = create_connecting_endpoint_from_config(
            link_config.transport["endpoint"],
            self._controller.cbdir,
            self._controller._reactor,
            self.log,
        )
        try:
