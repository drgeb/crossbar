...
class NativeProcess(ApplicationSession):
    ...
    def onJoin(self, details):
        ...
        self.log.info(
            'Registered {len_reg} management procedures on realm "{realm}"',
            len_reg=len(regs),
            realm=self.realm,
        )
        for reg in regs:
    ...
    def get_cpu_affinity(self, details=None):
        ...
        if not _HAS_PSUTIL:
            emsg = (
                "unable to get CPU affinity: required package 'psutil' is not installed"
            )
            self.log.warn(emsg)
            raise ApplicationError("crossbar.error.feature_unavailable", emsg)
    ...
    def set_cpu_affinity(self, cpus, relative=True, details=None):
        ...
        if not _HAS_PSUTIL:
            emsg = (
                "Unable to set CPU affinity: required package 'psutil' is not installed"
            )
            self.log.warn(emsg)
            raise ApplicationError("crossbar.error.feature_unavailable", emsg)
    ...
    def get_process_info(self, details=None):
        ...
        if self._pinfo:
            ...
            emsg = (
                "Could not retrieve process statistics: required packages not installed"
            )
            raise ApplicationError("crossbar.error.feature_unavailable", emsg)
    ...
    def start_manhole(self, config, details=None):
        ...
        * ``crossbar.node.<node_id>.controller.on_manhole_started`` - for node controllers
        ...
        """
        self.log.debug(
            "{cls}.start_manhole(config = {config})",
            cls=self.__class__.__name__,
            config=config,
        )
        ...
        self.publish(
            started_topic, started_info, options=PublishOptions(exclude=details.caller)
        )
    ...
    def stop_manhole(self, details=None):
        ...
        * ``crossbar.node.<node_id>.controller.on_manhole_stopped`` for node controllers
        ...
        stopped_info = None
        self.publish(
            stopped_topic, stopped_info, options=PublishOptions(exclude=details.caller)
        )
