...
def extract_peer_certificate(transport):
    ...
    if not _HAS_TLS:
        raise Exception(
            "cannot extract certificate - TLS support packages not installed"
        )

    # check if the Twisted transport is a TLSMemoryBIOProtocol
    if not (
        hasattr(transport, "getPeerCertificate") and ISSLTransport.providedBy(transport)
    ):
        return None
...
def _create_tls_server_context(config, cbdir, log):
    ...
    with open(key_filepath) as key_file:
        ...
        log.info(
            "Loading server TLS certificate from {cert_filepath}",
    ...
    if False:
        ...
        if ctx._ecCurve is None:
            ...
            if ctx._ecCurve.snName != "prime256v1":
                log.info(
                    "OpenSSL is using elliptic curve {curve}", curve=ctx._ecCurve.snName
                )
            else:
...
def _create_tls_client_context(config, cbdir, log):
    ...
    if "ca_certificates" in config:
        log.info(
            "TLS client using explicit trust ({cnt_certs} certificates)",
            cnt_certs=len(config["ca_certificates"]),
        )
        ca_certs = []
        for cert_fname in [
            os.path.abspath(os.path.join(cbdir, x)) for x in (config["ca_certificates"])
        ]:
            cert = crypto.load_certificate(
                crypto.FILETYPE_PEM, open(cert_fname, "rb").read()
            )
            log.info(
                "TLS client trust root CA certificate loaded from '{fname}'",
                fname=cert_fname,
            )
    ...
    if "key" in config:
        ...
        with open(cert_fname, "r") as f:
            ...
            log.info(
                "Loaded client TLS certificate from '{cert_fname}' (cn='{cert_cn}', sha256={cert_sha256}..)",
    ...
    ctx = optionsForClientTLS(
        hostname, trustRoot=ca_certs, clientCertificate=client_cert
    )
...
def create_listening_endpoint_from_config(config, cbdir, reactor, log):
        ...
        if "tls" in config:
            ...
            if _HAS_TLS:
                ...
                if version == 4:
                    endpoint = SSL4ServerEndpoint(
                        reactor, port, context, backlog=backlog, interface=interface
                    )
                elif version == 6:
            ...
            if version == 4:
                endpoint = TCP4ServerEndpoint(
                    reactor, port, backlog=backlog, interface=interface
                )
            elif version == 6:
                endpoint = TCP6ServerEndpoint(
                    reactor, port, backlog=backlog, interface=interface
                )
            else:
                raise Exception("invalid TCP protocol version {}".format(version))
    ...
    #
        ...
        if path.exists():
            log.info(
                (
                    "{path} exists, attempting to remove before using as a "
                    "UNIX socket"
                ),
                path=path,
            )
...
def create_listening_port_from_config(config, cbdir, factory, reactor, log):
    ...
    if config["type"] == "tcp" and config.get("shared", False):
        ...
        if "tls" in config:
            ...
            listening_port = SharedPort(
                port, factory, backlog, interface, reactor, shared=shared
            )
