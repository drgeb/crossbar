...
def _read_release_key():
    release_pubkey_file = "crossbar-{}.pub".format(
        "-".join(crossbar.__version__.split(".")[0:2])
    )
    release_pubkey_path = os.path.join(
        pkg_resources.resource_filename("crossbar", "common/keys"), release_pubkey_file
    )

    release_pubkey_hex = binascii.b2a_hex(
        cryptosign._read_signify_ed25519_pubkey(release_pubkey_path)
    ).decode("ascii")

    with open(release_pubkey_path) as f:
        release_pubkey_base64 = f.read().splitlines()[1]

    release_pubkey_qrcode = cryptosign._qrcode_from_signify_ed25519_pubkey(
        release_pubkey_path
    )
...
def _parse_key_file(key_path, private=True):
    ...
    allowed_tags = ["public-key-ed25519", "machine-id", "created-at", "creator"]
    if private:
...
def _read_node_key(
    cbdir, privkey_path="key.priv", pubkey_path="key.pub", private=False
):
    if private:
    ...
    if os.path.exists(privkey_path):
        ...
        for tag in [
            ...
            if tag not in priv_tags:
                raise Exception(
                    "Corrupt node private key file {} - {} tag not found".format(
                        privkey_path, tag
                    )
                )
        ...
        if os.path.exists(pubkey_path):
            ...
            for tag in ["creator", "created-at", "machine-id", "public-key-ed25519"]:
                if tag not in pub_tags:
                    raise Exception(
                        "Corrupt node public key file {} - {} tag not found".format(
                            pubkey_path, tag
                        )
                    )
        ...
        log.info(
            "New node key pair generated! Public key is {pubkey}",
            pubkey=hlid("0x" + pubkey_hex),
        )
