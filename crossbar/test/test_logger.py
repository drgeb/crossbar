...
class CrossbarLoggerTests(TestCase):
    ...
    def test_logger_emits(self):
        ...
        log._logger.emit.assert_called_with(
            LogLevel.debug, "Stuff that's trace", foo="bar", txaio_trace=1
        )
    ...
    def test_logger_namespace_init(self):
        ...
        self.assertEqual(
            lm.log._logger.namespace, "crossbar.test.test_logger._InitLoggerMaker"
        )
    ...
    def test_logger_namespace_classdef(self):
        ...
        self.assertEqual(
            lm.log._logger.namespace, "crossbar.test.test_logger._ClassDefLoggerMaker"
        )
...
class StdoutObserverTests(TestCase):
    ...
    def test_output_standard(self):
        ...
        result = stream.getvalue()
        self.assertEqual(
            result[:-1], formatTime(event["log_time"]) + " [foo] Hi there!"
        )
