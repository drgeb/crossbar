...
class NodeOptions(object):
    def __init__(
        self, debug_lifecycle=False, debug_programflow=False, enable_vmprof=False
    ):
...
class Node(object):
    ...
    def __init__(
        self, personality, cbdir=None, reactor=None, native_workers=None, options=None
    ):
        """
    ...
    def _set_shutdown_triggers(self, controller_options):
        ...
        if "shutdown" in controller_options:
            ...
            self._node_shutdown_triggers = [NODE_SHUTDOWN_ON_WORKER_EXIT]
            self.log.info(
                "Using default node shutdown triggers {triggers}",
                triggers=self._node_shutdown_triggers,
            )
    ...
    def start(self, node_id=None):
        ...
        """
        self.log.info(
            "Starting {personality} node {method}",
        ...
        self.log.info(
            "Node ID {node_id} set from {node_id_source}",
        ...
        self.log.debug(
            "Router service agent session attached [{router_service}]",
            router_service=qual(self.ROUTER_SERVICE),
        )

        self._router_session_factory.add(
            self._controller, router, authid="nodecontroller", authrole="trusted"
        )
        self.log.debug(
            "Node controller session attached [{node_controller}]",
    ...
    def boot_from_config(self, config):
        ...
        self.log.info(
            "{bootmsg} {method}",
            bootmsg=hl(
                "Booting node from local configuration [parallel_worker_start={}] ..".format(
        ...
        if len(workers):
            ...
            self.log.info(
                hl("No workers configured, nothing to do", color="green", bold=True)
            )
        ...
        for worker in workers:
            ...
            if worker_type in self._native_workers:
                ...
                worker_logname = "{} {}".format(
                    self._native_workers[worker_type]["logname"], hlid(worker_id)
                )
                ...
                if worker_disabled:
                    ...
                    def configure_worker(
                        res, worker_logname, worker_type, worker_id, worker
                    ):
                        self.log.info(
                    ...
                    d.addCallback(
                        configure_worker, worker_logname, worker_type, worker_id, worker
                    )
        ...
        self.log.info(
            hl(
                "Ok, local node configuration booted successfully!",
                color="green",
                bold=True,
            )
        )
    ...
    def _configure_native_worker_common(self, worker_logname, worker_id, worker):
        ...
        if False:
            if "pythonpath" in worker_options:
                ...
                self.log.warn(
                    "{worker}: PYTHONPATH extended for {paths}",
        ...
        if True:
            if "cpu_affinity" in worker_options:
                new_affinity = yield self._controller.call(
                    "crossbar.worker.{}.set_cpu_affinity".format(worker_id),
                    worker_options["cpu_affinity"],
                    options=CallOptions(),
                )
                self.log.debug(
                    "{worker}: CPU affinity set to {affinity}",
                    worker=worker_logname,
    ...
    def _configure_native_worker_router(self, worker_logname, worker_id, worker):
        ...
        for realm in worker.get("realms", []):
            ...
            yield self._controller.call(
                "crossbar.worker.{}.start_router_realm".format(worker_id),
                realm_id,
                realm,
                options=CallOptions(),
            )
            ...
            for role in realm.get("roles", []):
                ...
                yield self._controller.call(
                    "crossbar.worker.{}.start_router_realm_role".format(worker_id),
                    realm_id,
                    role_id,
                    role,
                    options=CallOptions(),
        ...
        for component in worker.get("components", []):
            ...
            yield self._controller.call(
                "crossbar.worker.{}.start_router_component".format(worker_id),
                component_id,
                component,
                options=CallOptions(),
            )
        ...
        for transport in worker.get("transports", []):
            ...
            yield self._controller.call(
                "crossbar.worker.{}.start_router_transport".format(worker_id),
                transport_id,
                transport,
                ...
                if paths:
                    for path in sorted(paths):
                        if path != "/":
                            ...
                            yield self._controller.call(
                                "crossbar.worker.{}.start_web_transport_service".format(
                                    worker_id
                                ),
                                transport_id,
                                path,
        ...
        for realm in worker.get("realms", []):
            ...
            for i, rlink in enumerate(realm.get("rlinks", [])):
                ...
                d = self._controller.call(
                    "crossbar.worker.{}.start_router_realm_link".format(worker_id),
                    realm_id,
                    rlink_id,
                    rlink,
                    options=CallOptions(),
    ...
    def _configure_native_worker_container(self, worker_logname, worker_id, worker):
        ...
        for component in worker.get("components", []):
            ...
            yield self._controller.call(
                "crossbar.worker.{}.start_component".format(worker_id),
                component_id,
                component,
                options=CallOptions(),
            )
    ...
    @inlineCallbacks
    def _configure_native_worker_websocket_testee(
        self, worker_logname, worker_id, worker
    ):
        yield self._configure_native_worker_common(worker_logname, worker_id, worker)
        ...
        yield self._controller.call(
            "crossbar.worker.{}.start_websocket_testee_transport".format(worker_id),
            transport_id,
            transport,
            options=CallOptions(),
        )
    ...
    def _configure_native_worker_proxy(self, worker_logname, worker_id, worker):
        ...
        for i, transport in enumerate(worker.get("transports", [])):
            ...
            yield self._controller.call(
                "crossbar.worker.{}.start_proxy_transport".format(worker_id),
                transport_id,
                transport,
                options=CallOptions(),
            )
