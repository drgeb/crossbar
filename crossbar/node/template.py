...
class Templates:
    ...
    def init(appdir, template="default", params=None, dryrun=False, skip_existing=True):
        ...
        if not template:
            raise Exception(
                'no such application directory template: "{}"'.format(template)
            )
        ...
        try:
            ...
            log.failure(
                "Something went wrong while instantiating app template - rolling back changes .."
            )
            for ptype, path in reversed(created):
                if ptype == "file":
                    ...
                    try:
                        ...
                        log.warn(
                            "Warning: could not remove directory {path}", path=path
                        )
                else:
