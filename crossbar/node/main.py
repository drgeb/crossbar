...
def _check_pid_exists(pid):
    ...
    if sys.platform == "win32":
        if _HAS_PSUTIL:
            ...
            # We use psutil.
            raise Exception(
                "cannot check if process with PID exists - package psutil not installed"
            )
    else:
        # Unix-like OS
...
def _check_is_running(cbdir):
            ...
            try:
                ...
                if pid == os.getpid():
                    ...
                    if pid_exists:
                        if _HAS_PSUTIL:
                            ...
                            if not _is_crossbar_process(cmdline):
                                ...
                                if len(nicecmdline) > 76:
                                    nicecmdline = (
                                        nicecmdline[:38] + " ... " + nicecmdline[-38:]
                                    )
                                log.info(
                                    '"{fp}" points to PID {pid} which is not a crossbar process:',
                                    fp=fp,
                                    pid=pid,
                                )
                                log.info("  {cmdline}", cmdline=nicecmdline)
                                log.info(
                                    "Verify manually and either kill {pid} or delete {fp}",
                                    pid=pid,
                                    fp=fp,
                                )
                                return None
                        ...
                        remove_PID_reason = (
                            "pointing to non-existing process with PID {}".format(pid)
                        )
    ...
    if remove_PID_type:
        ...
        try:
            ...
            log.info(
                (
                    "Could not remove {pidtype} Crossbar.io PID file "
...
def _run_command_version(options, reactor, personality):
    ...
    log.info(
        " Frozen executable  : {py_is_frozen}",
...
def _run_command_keys(options, reactor, personality):
    ...
    log.info(
        "{key_title}",
        key_title=hl("Crossbar Software Release Key", color="yellow", bold=True),
    )
...
def _run_command_init(options, reactor, personality):
    ...
    if os.path.exists(options.appdir):
        log.warn(
            "Application directory '{appdir}' already exists!", appdir=options.appdir
        )
    else:
        try:
            ...
            log.info(
                "Crossbar.io application directory '{appdir}' created",
                appdir=options.appdir,
            )

    log.info(
        "Initializing application directory '{options.appdir}' ..", options=options
    )
...
def _run_command_status(options, reactor, personality):
    ...
    if pid_data is None:
        if _assert == "running":
            log.error(
                "Assert status RUNNING failed: status is {}".format(
            ...
            log.info(
                "Assert status STOPPED succeeded: status is {}".format(
        ...
        if _assert == "running":
            log.info(
                "Assert status RUNNING succeeded: status is {}".format(
            ...
            log.error(
                "Assert status STOPPED failed: status is {}".format(
...
def _run_command_stop(options, reactor, personality):
    ...
    if pid_data:
        pid = pid_data["pid"]
        print(
            "Stopping Crossbar.io currently running from node directory {} (PID {}) ...".format(
                options.cbdir, pid
            )
        )
        if not _HAS_PSUTIL:
            ...
            try:
                ...
                print(
                    "SIGINT sent to process {} .. waiting for exit ({} seconds) ...".format(
                        pid, _INTERRUPT_TIMEOUT
                    )
                )
        ...
        print(
            "No Crossbar.io is currently running from node directory {}.".format(
                options.cbdir
            )
        )
...
def _start_logging(options, reactor):
    ...
    if getattr(options, "logtofile", False):
        ...
        if loglevel == "none":
            ...
            observers.append(
                make_stdout_observer(show_source=False, format=logformat, color=color)
            )
            observers.append(
                make_stderr_observer(show_source=False, format=logformat, color=color)
            )
        elif loglevel == "debug":
            ...
            observers.append(
                make_stderr_observer(show_source=True, format=logformat, color=color)
            )
        elif loglevel == "trace":
    ...
    for observer in observers:
        ...
        reactor.addSystemEventTrigger(
            "after", "shutdown", globalLogPublisher.removeObserver, observer
        )
...
def _run_command_start(options, reactor, personality):
    ...
    if pid_data:
        print(
            "Crossbar.io is already running from node directory {} (PID {}).".format(
                options.cbdir, pid_data["pid"]
            )
        )
        sys.exit(1)
    else:
        ...
        with open(fp, "wb") as fd:
            ...
            pid_data = {
                ...
                "argv": argv,
                "options": {
                    x: y for x, y in options_dump.items() if x not in ["func", "argv"]
                },
            }
            fd.write(
                "{}\n".format(
    ...
    node = personality.Node(
        personality, options.cbdir, reactor=reactor, options=node_options
    )
    ...
    log.info("")
    log.info(
        "Initializing {node_class} as node [realm={realm}, cbdir={cbdir}]",
    ...
    def after_reactor_started():
        ...
        if _HAS_VMPROF and options.vmprof:
            outfn = os.path.join(
                options.cbdir, ".vmprof-controller-{}.dat".format(os.getpid())
            )
    ...
    def start_crossbar():
        ...
        def on_startup_success(_shutdown_complete):
            ...
            def on_shutdown_success(shutdown_info):
                ...
                log.info(
                    "on_shutdown_success: was_clean={was_clean}",
                    shutdown_info["was_clean"],
                )
...
def _print_usage(prog, personality):
    ...
    print(
        'Type "{} --help" to get help, or "{} <command> --help" to get help on a specific command.'.format(
            prog, prog
        )
    )
    print(
        'Type "{} legal" to read legal notices, terms of use and license and privacy information.'.format(
            prog
        )
    )
